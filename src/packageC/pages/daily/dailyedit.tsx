              ...
              let newArr = allData.map((item: any) => {
                return Object.assign(
                  {},
                  {
                    ...
                    filePath:
                      item[0]['fileURL'].split('?Expires=').length > 0
                        ? item[0]['fileURL'].split('?Expires=')[0]
                        : item[0]['fileURL'],
                    id: item[0]['id'],
                    url:
                      item[0]['fileURL'].split('?Expires=').length > 0
          ...
          Promise.all(promises)
            .then(allData => {
              ...
              const newDailyProjectPhotos = ddDocumentsTmp
                .map((ddtItem, index) => {
                  let tmpDDitem = Object.assign(ddtItem, { place: index + 1 })
      ...
      if (res === null) {
        ...
        if (
          ...
          const tmpDD = res.dailyProjectPhotos
            ...
            .filter(item => {
              return !!item.url || !!item.imageDocs
            })
        ...
        this.setState({
          ...
          planOverTime:
            res.planOverTime === 0
              ? '无'
              : moment(res.planOverTime).format(dateFormat),
          todayContents:
                  ...
                  <View className="at-col">
                    <View className="vc pl30">
                      ...
                      <AtInput
                        border={false}
                        type="number"
                        value={parseInt(this.state.workersCount + '') + ''}
